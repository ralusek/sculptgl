...
var readScalarValues = function(x, voxels, grid, dims, n, cols, mats) {
  ...
  //     for (var i = 0; i < 2; ++i) {
    for(var i=0; i<8; ++i) {
        ...
        // var id = n + cubeVerts[i][0] + cubeVerts[i][1] * rx + cubeVerts[i][2] * rxy;
        var v = cubeVerts[i];
        var id = (x[0]+v[0]) + (x[1]+v[1]) * rx + (x[2]+v[2]) * rxy;
  ...
  for (x[2] = 0; x[2] < dims[2] - 1; ++x[2], n += dims[0]) {
    for (x[1] = 0; x[1] < dims[1] - 1; ++x[1], ++n) {
      for (x[0] = 0; x[0] < dims[0] - 1; ++x[0], ++n) {
        var cubeIndex = readScalarValues(x, voxels, grid, dims, n, cols, mats);
        ...
        for (var k = 0; k < 12; ++k) {
          ...
          for (var h = 0; h < 3; ++h) {
            nv[h] = x[h] + p0[h] + t * (p1[h] - p0[h]);
            // if(nv[h] !== nv[h])debugger;
            // if(nv[h] === Infinity ||nv[h] === -Infinity)debugger;
          }
        ...
        for (var l = 0; l < f.length; l += 3) {
          faces.push(edges[f[l]], edges[f[l + 1]], edges[f[l + 2]], 4294967295);
        }
      }
    }
  }
